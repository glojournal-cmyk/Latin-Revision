...
function runAction(action, element) {
  if (!action) return;
  try {
        if (action === 'open-games') return window.GameV2 ? window.GameV2.openHub() : showRouteError('Games are still loading. Please try again.');
    if (action === 'game-forge') return window.GameV2 ? window.GameV2.start('forma') : showRouteError('Games are still loading.');
    if (action === 'game-mosaic') return window.GameV2 ? window.GameV2.start('mosaic') : showRouteError('Games are still loading.');
    if (action === 'game-verbum') return window.GameV2 ? window.GameV2.start('verbum') : showRouteError('Games are still loading.');
    if (action === 'game-manuscript') return window.GameV2 ? window.GameV2.start('manuscript') : showRouteError('Games are still loading.');
    if (action === 'games-back') return window.GameV2 ? window.GameV2.exit() : show('gamesHub');
...
function renderV10ProgressExtras(){
  ...
  let best=0;
  try{
    const gamesV2=state.gamesV2&&state.gamesV2.games?state.gamesV2.games:{};
    Object.values(gamesV2).forEach(game=>{
      const values=game&&game.bestPoints?Object.values(game.bestPoints):[];
      values.forEach(value=>best=Math.max(best,Number(value)||0));
    });
  }catch(e){}
