...
let sessionInfo = {};
let sessionAnswers = [];
let lastExplanationContext = null;
...
function checkAnswer() {
  ...
  html += `<div class="explanation-box"><strong>Teacher-bank explanation</strong><ol class="feedback-list">${(question.steps || [question.explain || 'Compare with the accepted answer.']).map(step => `<li>${esc(step)}</li>`).join('')}</ol>${question.explain ? `<div class="small muted" style="margin-top:8px">${esc(question.explain)}</div>` : ''}<div class="ai-explain-row"><button class="ai-explain-button" type="button" data-action="ask-ai-explain">✨ Ask AI to explain differently ↗</button><span class="small muted">AI is supplementary; the accepted answer above stays teacher-locked.</span></div></div>`;
  ...
  lastExplanationContext = {
    ...
    direction: question.direction || question.topic || '',
    given,
    accepted: answer,
    explain: question.explain || '',
    steps: Array.isArray(question.steps) ? question.steps : [],
    mustRemember: question.mustRemember || ''
  };
...
function askAIExplain(){
  const c=lastExplanationContext;
  if(!c){
    return showRouteError('Answer the current question first, then ask AI to explain it.');
  }
  const prompt=[
    'You are helping a Year 8 student understand one Latin revision question.',
    'The accepted answer below comes from the teacher-locked revision bank. Do not replace or contradict it.',
    'Explain the reasoning in simple, encouraging steps. Point out exactly why the student answer differs.',
    'Keep the explanation concise and age-appropriate. Do not introduce grammar or vocabulary outside what is needed for this question.',
    '',
    'Direction: '+c.direction,
    c.context ? 'Context / clue: '+c.context : '',
    'Question: '+c.question,
    'Student answer: '+c.given,
    'Teacher-locked accepted answer: '+c.accepted,
    c.steps.length ? 'Teacher steps: '+c.steps.join(' | ') : '',
    c.explain ? 'Teacher explanation: '+c.explain : '',
    c.mustRemember ? 'Must remember: '+c.mustRemember : ''
  ].filter(Boolean).join('\n');
  ...
  if(!opened){
    navigator.clipboard?.writeText(prompt).then(()=>{
      alert('AI explanation prompt copied. Open ChatGPT and paste it there.');
    }).catch(()=>{
      alert('Open ChatGPT and ask it to explain this question using the teacher-locked accepted answer.');
    });
  }
}
...
function soundPluck(audio,frequency,when=.0,volume=.04,duration=.28){
  ...
  overtone.frequency.setValueAtTime(frequency*2.01,t);
  filter.type='lowpass';
  ...
  osc.stop(t+duration+.02); overtone.stop(t+duration+.02);
}

function soundWood(audio,when=.0,volume=.035,pitch=170){
  const t=audio.currentTime+when;
  const osc=audio.createOscillator();
  const gain=audio.createGain();
  const filter=audio.createBiquadFilter();
  ...
  osc.frequency.exponentialRampToValueAtTime(Math.max(80,pitch*.62),t+.09);
  filter.type='lowpass'; filter.frequency.setValueAtTime(700,t);
  gain.gain.setValueAtTime(.0001,t);
  gain.gain.exponentialRampToValueAtTime(volume,t+.004);
...
function soundPaper(audio,when=.0,volume=.014){
  ...
  const t=audio.currentTime+when;
  src.buffer=buffer;
  high.type='bandpass'; high.frequency.value=1800; high.Q.value=.7;
  gain.gain.setValueAtTime(volume,t);
  gain.gain.exponentialRampToValueAtTime(.0001,t+.075);
  src.connect(high).connect(gain).connect(audio.destination);
  src.start(t);
}
...
if (action === 'home') return showDashboard();
    ...
    if (action === 'ask-ai-explain') return askAIExplain();
...
let latinGame = {type:null,score:0,combo:0,maxCombo:0,round:0,timer:null,seconds:0,answer:'',selected:[],pairs:[],attempts:0,correct:0,seen:[],boosts:0,startedAt:0};
...
function updateLatinGameStats(){
  const s=document.getElementById('gameScore'),c=document.getElementById('gameCombo'),
        t=document.getElementById('gameTimer'),r=document.getElementById('gameRound'),
        fill=document.getElementById('gameProgressFill'),mood=document.getElementById('gameMood'),
        sound=document.getElementById('gameSoundButton');

  if(s){
    if(latinGame.type==='sprint') s.textContent='Points '+latinGame.score;
    else if(latinGame.type==='match' && latinGame.levelId){
      const hearts=Math.max(0,(latinGame.maxMistakes||0)-(latinGame.mistakes||0));
      s.textContent='Score '+latinGame.score+'/100 · ♥ '+hearts;
    }else s.textContent='Score '+latinGame.score+'/100';
  }
  if(c){
  ...
  if(r){
    if(latinGame.type==='sprint') r.textContent='Answered '+latinGame.round;
    else if(latinGame.type==='match'&&latinGame.levelId) r.textContent='Level '+latinGame.levelId+' · '+latinGame.round+'/'+total+' pairs';
    else if(total) r.textContent='Round '+Math.min(latinGame.round+1,total)+' / '+total;
    else r.textContent='';
  }
  if(fill){
    const pct=latinGame.type==='sprint'?Math.min(100,Math.max(0,(60-latinGame.seconds)/60*100)):(total?Math.min(100,latinGame.round/total*100):0);
    fill.style.width=pct+'%';
  }
  if(mood){
    if(latinGame.type==='match'&&latinGame.levelId){
      const hearts=Math.max(0,(latinGame.maxMistakes||0)-(latinGame.mistakes||0));
      mood.textContent=latinGame.combo>=3?'✨ Match streak ×'+latinGame.combo:'Clear every pair · '+hearts+' heart'+(hearts===1?'':'s')+' left';
    }else{
...
function startLatinGame(type){
  ...
  latinGame={type,score:0,combo:0,maxCombo:0,round:0,timer:null,seconds:type==='sprint'?60:0,
    answer:'',selected:[],pairs:[],attempts:0,correct:0,seen:[],boosts:0,startedAt:Date.now(),
    levelId:0,levelName:'',mistakes:0,maxMistakes:0,failed:false,stars:0};

  const titles={match:'Verbum Match',gladiator:'Gladiator Challenge',sentence:'Build the Sentence',sprint:'Roman Sprint'};
  ...
  if(type==='match') gameSafeRender(renderMatchLevelSelect);
  else if(type==='sentence') gameSafeRender(renderLatinSentence);
  else if(type==='sprint') startSprintCountdown();
  else gameSafeRender(renderLatinChallenge);
}
...
const MATCH_LEVELS=[
  {id:1,name:'Novice I',pairs:4,hearts:3,subtitle:'Warm-up board'},
  {id:2,name:'Novice II',pairs:5,hearts:3,subtitle:'Build confidence'},
  {id:3,name:'Scholar I',pairs:6,hearts:3,subtitle:'Steady recall'},
  {id:4,name:'Scholar II',pairs:6,hearts:2,subtitle:'Fewer mistakes allowed'},
  {id:5,name:'Magister I',pairs:7,hearts:2,subtitle:'Longer challenge'},
  {id:6,name:'Magister II',pairs:8,hearts:2,subtitle:'Mastery board'}
];

function ensureMatchCampaign(){
  state.games=state.games||{};
  state.games.verbumMatch=state.games.verbumMatch||{unlocked:1,best:{}};
  const c=state.games.verbumMatch;
  c.unlocked=Math.max(1,Math.min(MATCH_LEVELS.length,Number(c.unlocked)||1));
  c.best=c.best||{};
  return c;
}
...
function renderMatchLevelSelect(){
  const area=document.getElementById('gameArea');
  const title=document.getElementById('gameTitle');
  if(title) title.textContent='Verbum Match';
  const campaign=ensureMatchCampaign();

  latinGame.levelId=0; latinGame.levelName=''; latinGame.pairs=[]; latinGame.round=0;
  latinGame.score=0; latinGame.combo=0; latinGame.mistakes=0; latinGame.maxMistakes=0;
  updateLatinGameStats();

  area.innerHTML='<div class="game-question"><div class="game-round-kicker">VERBUM MATCH · LEVEL MODE</div>'+
    '<h3>Clear levels and unlock the next challenge.</h3>'+
    '<p>Every level uses real Latin → English pairs from the teacher-locked question bank.</p>'+
    '<div class="match-level-grid">'+MATCH_LEVELS.map(level=>{
      const locked=level.id>campaign.unlocked;
      const best=campaign.best[level.id]||null;
      ...
      return '<button type="button" class="match-level-card'+(locked?' locked':'')+'" data-match-level="'+level.id+'" '+(locked?'disabled':'')+'>'+
        ...
        '<h4>'+gameEsc(level.name)+'</h4><p>'+gameEsc(level.subtitle)+'</p>'+
        '<div class="match-level-meta"><span>'+level.pairs+' pairs</span><span>'+level.hearts+' hearts</span></div>'+
        '<strong class="match-level-cta">'+(locked?'🔒 Locked':'Play level →')+'</strong></button>';
    }).join('')+'</div><div class="match-level-note">Unlocked: '+campaign.unlocked+' / '+MATCH_LEVELS.length+' levels</div></div>';

  area.querySelectorAll('[data-match-level]').forEach(btn=>btn.addEventListener('click',()=>prepareLatinMatchLevel(Number(btn.dataset.matchLevel))));
}

function prepareLatinMatchLevel(levelId){
  const level=MATCH_LEVELS.find(x=>x.id===levelId);
  const campaign=ensureMatchCampaign();
  if(!level||level.id>campaign.unlocked) return;

  const pool=gameShuffle(latinVocabPairs());
  if(pool.length<level.pairs){
    latinGameMessage('Not enough matching vocabulary was found in the teacher-locked source bank.');
    return;
  }

  latinGame.levelId=level.id; latinGame.levelName=level.name; latinGame.pairs=pool.slice(0,level.pairs);
  latinGame.round=0; latinGame.score=0; latinGame.combo=0; latinGame.maxCombo=0;
  latinGame.attempts=0; latinGame.correct=0; latinGame.mistakes=0; latinGame.maxMistakes=level.hearts;
  latinGame.failed=false; latinGame.stars=0; latinGame.selected=[];

  const title=document.getElementById('gameTitle');
  if(title) title.textContent='Verbum Match · '+level.name;
  playTone('countdown');
  updateLatinGameStats();
  renderLatinMatch();
}
...
function renderLatinMatch(){
  if(!latinGame.levelId||!latinGame.pairs.length){renderMatchLevelSelect();return;}

  const tiles=gameShuffle(latinGame.pairs.flatMap((p,index)=>[
    {pair:index,side:'latin',text:p.latin},{pair:index,side:'english',text:p.english}
  ]));
  const area=document.getElementById('gameArea');
  area.innerHTML='<div class="game-question"><div class="game-round-kicker">LEVEL '+latinGame.levelId+' · '+gameEsc(latinGame.levelName.toUpperCase())+'</div>'+
    '<h3>Match each Latin word with its English meaning.</h3><p>Clear the whole board before you run out of hearts.</p></div>'+
    '<div class="match-level-bar"><span>'+gameEsc(latinGame.levelName)+'</span><span class="match-hearts">'+'♥'.repeat(Math.max(0,latinGame.maxMistakes-latinGame.mistakes))+'</span></div>'+
    '<div class="match-grid">'+tiles.map(x=>'<button class="match-tile" type="button" data-pair="'+x.pair+'" data-side="'+x.side+'">'+gameEsc(x.text)+'</button>').join('')+'</div>';

  area.querySelectorAll('.match-tile').forEach(btn=>btn.addEventListener('click',()=>pickLatinMatch(btn)));
}

function pickLatinMatch(button){
  if(button.classList.contains('matched')||button.disabled) return;

  const selected=document.querySelector('.match-tile.selected');
  if(!selected){button.classList.add('selected');playTone('tile');return;}
  if(selected===button){selected.classList.remove('selected');return;}

  if(selected.dataset.side===button.dataset.side){
    selected.classList.remove('selected');button.classList.add('selected');playTone('tile');return;
  }

  selected.classList.remove('selected');
  const correct=selected.dataset.pair===button.dataset.pair&&selected.dataset.side!==button.dataset.side;
  latinGame.attempts++;

  if(correct){
    selected.classList.add('matched','pop'); button.classList.add('matched','pop');
    selected.disabled=true; button.disabled=true;
    latinGame.correct++; latinGame.combo++; latinGame.maxCombo=Math.max(latinGame.maxCombo,latinGame.combo); latinGame.round++;
    latinGame.score=Math.round((latinGame.correct/Math.max(1,latinGame.pairs.length))*100);
    updateLatinGameStats();
    playTone(latinGame.combo>=3?'combo':'match');
    gameBurst(latinGame.combo>=3?'COMBO ×'+latinGame.combo+' ✨':'Matched!','good');
    if(latinGame.round>=latinGame.pairs.length) setTimeout(()=>finishLatinMatchLevel(true),350);
  }else{
    latinGame.combo=0; latinGame.mistakes++;
    latinGame.score=Math.round((latinGame.correct/Math.max(1,latinGame.pairs.length))*100);
    updateLatinGameStats(); playTone('wrong');
    selected.classList.add('shake'); button.classList.add('shake'); gameBurst('Wrong pair · ♥ -1','soft');
    ...
    if(latinGame.mistakes>=latinGame.maxMistakes){
      ...
      setTimeout(()=>finishLatinMatchLevel(false),420);
    }
  }
}

function finishLatinMatchLevel(passed){
  const area=document.getElementById('gameArea');
  const campaign=ensureMatchCampaign();

  if(passed){
    latinGame.score=100;
    latinGame.stars=latinGame.mistakes===0?3:(latinGame.mistakes===1?2:1);
    const previous=campaign.best[latinGame.levelId]||{stars:0,score:0};
    campaign.best[latinGame.levelId]={stars:Math.max(previous.stars||0,latinGame.stars),score:100};
    campaign.unlocked=Math.min(MATCH_LEVELS.length,Math.max(campaign.unlocked,latinGame.levelId+1));
    save(); playTone('victory');

    const nextAvailable=latinGame.levelId<MATCH_LEVELS.length;
    area.innerHTML='<div class="game-result match-level-result"><img src="'+LATIN_GAME_ART.gold+'" alt="">'+
      '<div class="game-round-kicker">LEVEL '+latinGame.levelId+' CLEARED</div><h2>'+gameEsc(latinGame.levelName)+' complete!</h2>'+
      '<div class="level-stars-big">'+'★'.repeat(latinGame.stars)+'☆'.repeat(3-latinGame.stars)+'</div>'+
      '<p>Board cleared: <strong>100/100</strong></p><div class="result-stats">'+
      '<span><strong>'+latinGame.correct+'</strong> pairs</span><span><strong>'+latinGame.mistakes+'</strong> mistakes</span><span><strong>'+latinGame.maxCombo+'</strong> best combo</span></div>'+
      '<p class="game-note">'+(nextAvailable?'Next level unlocked.':'All Verbum Match levels cleared!')+'</p>'+
      '<div class="game-result-actions"><button class="secondaryButton" type="button" id="matchReplayLevel">Replay level</button>'+
      (nextAvailable?'<button class="primaryButton" type="button" id="matchNextLevel">Next level →</button>':'')+
      '<button class="secondaryButton" type="button" id="matchLevelMenu">Level menu</button></div></div>';

    document.getElementById('matchReplayLevel').addEventListener('click',()=>prepareLatinMatchLevel(latinGame.levelId));
    const next=document.getElementById('matchNextLevel');
    if(next) next.addEventListener('click',()=>prepareLatinMatchLevel(latinGame.levelId+1));
    document.getElementById('matchLevelMenu').addEventListener('click',renderMatchLevelSelect);
  }else{
    playTone('wrong');
    area.innerHTML='<div class="game-result match-level-result"><img src="'+LATIN_GAME_ART.bronze+'" alt="">'+
      '<div class="game-round-kicker">LEVEL '+latinGame.levelId+'</div><h2>Hearts used up</h2>'+
      '<p>You matched <strong>'+latinGame.correct+' of '+latinGame.pairs.length+'</strong> pairs.</p>'+
      '<p class="game-note">Try the same level again — the vocabulary will reshuffle.</p>'+
      '<div class="game-result-actions"><button class="primaryButton" type="button" id="matchTryAgain">Try again</button>'+
      '<button class="secondaryButton" type="button" id="matchLevelMenu">Level menu</button></div></div>';
    document.getElementById('matchTryAgain').addEventListener('click',()=>prepareLatinMatchLevel(latinGame.levelId));
    document.getElementById('matchLevelMenu').addEventListener('click',renderMatchLevelSelect);
  }
}
...
function finishLatinGame(){
  ...
  area.innerHTML=
    ...
    '<div class="result-stats">'+
      ...
      (latinGame.type==='sprint' ? '<span><strong>'+latinGame.boosts+'</strong> time boosts</span>' : '')+
    '</div>'+
    '<div class="game-result-actions">'+
      '<button class="primaryButton" type="button" id="latinGameAgain">Play again</button> '+
      '<button class="secondaryButton" type="button" id="latinGameNext">Next game</button> '+
  ...
  document.getElementById('latinGameNext').addEventListener('click',()=>{
    const order=['match','gladiator','sentence','sprint'];
    startLatinGame(order[(order.indexOf(latinGame.type)+1)%order.length]);
  });
